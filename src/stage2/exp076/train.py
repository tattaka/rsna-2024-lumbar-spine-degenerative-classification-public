...
using axial prediciton (clean), using sagittal prediciton(clean)
"""
...
def get_transform(mode: str = "valid", img_size: int = 128):
    if mode == "train":
        transform = A.Compose(
            [
                A.RandomResizedCrop(
                    width=img_size, height=img_size, scale=(0.5, 1.5), p=0.9
                ),
                A.ShiftScaleRotate(
                    shift_limit=0.1,
                    scale_limit=0.0,
...
class RSNA2024Stage2Dataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        cropped_volume_st1 = crop_sagittal(
            volume,
            self.img_size_st1,
            self.in_chans_st1,
            keypoint,
        ...
        cropped_volume_st2 = crop_sagittal(
            volume,
            self.img_size_st2,
            self.in_chans_st2,
            keypoint,
        ...
        cropped_volume_ax_l = crop_axial(
            volume_ax,
            self.img_size_ax,
            self.in_chans_ax,
            keypoint_l,
        ...
        cropped_volume_ax_r = crop_axial(
            volume_ax,
            self.img_size_ax,
            self.in_chans_ax,
            keypoint_r,
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, input):
        ...
        if isinstance(input, dict):
            ...
            x = input
            mask = None
        # Attention スコアの計算 (batch_size, seq_length, 1)
        attention_scores = self.attention_weights(x)

        # ソフトマックス関数でスコアを正規化 (batch_size, seq_length, 1)
        if mask is not None:
...
class RSNA2024Stage2Model(nn.Module):
    def __init__(
        ...
        num_class: int = 3,
        model_name_s: str = "resnet34",
        drop_path_rate_s: float = 0,
        img_size_st1: int = 128,
        img_size_st2: int = 128,
        model_name_ax: str = "resnet34",
        drop_path_rate_ax: float = 0,
        img_size_ax: int = 256,
        ...
        if (
            ("swin" in model_name_s)
            or ("coat" in model_name_s)
            or ("max" in model_name_s)
        ):
            assert img_size_st1 == img_size_st2

        self.encoder_s = timm.create_model(
            model_name_s,
            ...
            img_size=(
                img_size_st1
                if ("swin" in model_name_s)
                or ("coat" in model_name_s)
                or ("max" in model_name_s)
                else None
            ),
        )
        self.model_name_s = model_name_s
        self.output_fmt_s = getattr(self.encoder_s, "output_fmt", "NHCW")
        num_features_s = self.encoder_s.feature_info.channels()
        self.neck_st1 = nn.Sequential(
            nn.Linear(num_features_s[-1] * 2, transformer_dim),
            nn.GELU(),
        ...
        self.encoder_ax = timm.create_model(
            model_name_ax,
            pretrained=pretrained,
            in_chans=1,
            features_only=True,
            drop_path_rate=drop_path_rate_ax,
            img_size=(
                img_size_ax
                if ("swin" in model_name_ax)
                or ("coat" in model_name_ax)
                or ("max" in model_name_ax)
                else None
            ),
        )
        self.model_name_ax = model_name_ax
        self.output_fmt_ax = getattr(self.encoder_ax, "output_fmt", "NHCW")
        num_features_ax = self.encoder_ax.feature_info.channels()

        self.neck_ax = nn.Sequential(
            nn.Linear(num_features_ax[-1] * 2, transformer_dim),
            nn.GELU(),
            nn.LayerNorm(transformer_dim),
        )
    ...
    def forward_image_feats_st1(self, img_st1):
        ...
        img_feats = self.encoder_s(img_st1)
        if self.output_fmt_s == "NHWC":
            img_feats = [
    ...
    def forward_image_feats_st2(self, img_st2):
        ...
        img_feats = self.encoder_s(img_st2)
        if self.output_fmt_s == "NHWC":
            img_feats = [
                img_feat.permute(0, 3, 1, 2).contiguous() for img_feat in img_feats
            ]
    ...
    def forward_image_feats_ax(self, img_ax):
        ...
        img_feats = self.encoder_ax(img_ax)
        if self.output_fmt_ax == "NHWC":
            img_feats = [
                img_feat.permute(0, 3, 1, 2).contiguous() for img_feat in img_feats
            ]
        img_feats = img_feats[-1]
    ...
    def forward_head(self, img_feats_st1, img_feats_st2, img_feats_ax, mask=None):
        img_feats = torch.cat(
            [
                img_feats_st1,
                img_feats_st2,
        ...
        img_feats = self.transformer(
            img_feats
            + self.pos_enc(torch.arange(img_feats.shape[1]))[None, :].to(
    ...
    def forward_aux_head(self, img_feats_st1, img_feats_st2, img_feats_ax):
        aux_logit = (
            [h(img_feats_st1) for h in self.aux_head[0:2]]
            + [h(img_feats_st2) for h in self.aux_head[2:3]]
            + [
                h(img_feats_ax[:, : img_feats_ax.shape[1] // 2])
                for h in self.aux_head[3:4]
            ]
            + [
                h(img_feats_ax[:, img_feats_ax.shape[1] // 2 :])
                for h in self.aux_head[4:5]
            ]
        )
        return aux_logit
    ...
    def forward(
        ...
        logit = self.forward_head(img_feats_st1, img_feats_st2, img_feats_ax)
        aux_logit = self.forward_aux_head(img_feats_st1, img_feats_st2, img_feats_ax)
...
class RSNA2024Stage2LightningModel(pl.LightningModule):
    def __init__(
        ...
        lr: float = 1e-3,
        backbone_s_lr: float = None,
        backbone_ax_lr: float = None,
        ...
        self.__build_model(
            ...
            drop_path_rate_s=drop_path_rate_s,
            img_size_st1=img_size_st1,
            img_size_st2=img_size_st2,
            model_name_ax=model_name_ax,
            drop_path_rate_ax=drop_path_rate_ax,
        ...
        self.hparams.backbone_s_lr = (
            self.hparams.backbone_s_lr if self.hparams.backbone_s_lr is not None else lr
        )
        self.hparams.backbone_ax_lr = (
            self.hparams.backbone_ax_lr
            if self.hparams.backbone_ax_lr is not None
            else lr
        )
        self.gt_val = []
        self.logit_val = []
    ...
    def calc_loss(self, outputs: torch.Tensor, labels: torch.Tensor):
        ...
        losses["aux"] = (
            ...
            + losses["ce_r_aux"] * 0.5
            + losses["ce_aux"]
            + losses["ce_l_ax_aux"] * 0.5
            + losses["ce_r_ax_aux"] * 0.5
        ) / 3
    ...
    def training_step(self, batch, batch_idx):
        ...
        volume_st1, volume_st2, volume_ax, label = (
            batch["volume_st1"],
            batch["volume_st2"],
            batch["volume_ax"],
            batch["label"],
        ...
        outputs.update(self.model(volume_st1, volume_st2, volume_ax))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        outputs.update(self.model_ema.module(volume_st1, volume_st2, volume_ax))
        loss_target["label"] = label
        losses = self.calc_loss(outputs, loss_target)
        ...
        optimizer_parameters = [
            {
                "params": [
                    p
                    for n, p in list(self.model.encoder_s.named_parameters())
                    if any(nd in n for nd in no_decay)
                ],
                "weight_decay": 0,
                "lr": self.hparams.backbone_s_lr,
            },
            {
                "params": [
                    p
            ...
            },
            {
                "params": [
                    p
                    for n, p in list(self.model.encoder_ax.named_parameters())
                    if any(nd in n for nd in no_decay)
                ],
                "weight_decay": 0,
                "lr": self.hparams.backbone_ax_lr,
            },
            {
                "params": [
                    p
...
def main(args):
    ...
    for fold in range(5):
        ...
        model = RSNA2024Stage2LightningModel(
            ...
            model_name_s=args.model_name_s,
            drop_path_rate_s=args.drop_path_rate_s,
            img_size_st1=args.img_size_st1,
            img_size_st2=args.img_size_st2,
            model_name_ax=args.model_name_ax,
            drop_path_rate_ax=args.drop_path_rate_ax,
            img_size_ax=args.img_size_ax,
            ...
            lr=args.lr,
            backbone_s_lr=args.backbone_s_lr,
            backbone_ax_lr=args.backbone_ax_lr,
